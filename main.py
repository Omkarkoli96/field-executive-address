...
Loan_ID in Customer Data, geocodes both addresses (with caching so
repeat addresses aren't re-geocoded), and computes the distance between
them:

  - Distance <= 1 km  -> VERIFIED
  - Distance >  1 km  -> NOT_VERIFIED
  - Either address couldn't be geocoded -> COULD_NOT_GEOCODE
...
def compare(
    ...
    for _, vrow in visited_df.iterrows():
        ...
        cust_coords = geocode_address(db, customer_address)
        visited_coords = geocode_address(db, visited_address)
